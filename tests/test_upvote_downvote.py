...
class TestQuestionAnswers(unittest.TestCase):
    ...
    def test_upvote_downvote_answer(self):
        ...
        #successful downvote
        result3 = self.client().post('/api/v1/questions/1/answers/1/downvote',
                                     headers=dict(Authorization="Bearer " + self.a_token3))
        result4 = self.client().post('/api/v1/questions/1/answers/1/downvote',
                                     headers=dict(Authorization="Bearer " + self.a_token3))
        my_data3 = json.loads(result3.data)
        self.assertEqual(result3.status_code, 200)
        self.assertEqual("Thanks for contributing!", my_data3["message"])
        #test user downvoting more than once
        my_data4 = json.loads(result4.data)
        self.assertEqual(result4.status_code, 200)
        self.assertEqual("Downvote already noted.", my_data4["message"])
        #non-existent question
        result5 = self.client().post('/api/v1/questions/100/answers/1/upvote',
                                     headers=dict(Authorization="Bearer " + self.a_token3))
        ...
        #non-exitent answer
        result6 = self.client().post('/api/v1/questions/1/answers/100/upvote',
                                     headers=dict(Authorization="Bearer " + self.a_token3))
        ...
        result6 = self.client().post('/api/v1/questions/1/answers/1/upvote',
                                     headers=dict(Authorization="Bearer " + self.a_token2))
        my_data6 = json.loads(result6.data)
        self.assertEqual(result6.status_code, 401)
        self.assertEqual("You cannot vote on your own answer.", my_data6["message"])
