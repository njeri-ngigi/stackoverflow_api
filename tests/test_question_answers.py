...
class TestQuestionAnswers(unittest.TestCase):
    ...
    def test_accept_update_answer(self):
        ...
        result = self.client().put('/api/v1/questions/1/answers/0',
                                   headers=dict(Authorization="Bearer " + self.a_token),
                                   content_type="application/json",
                                   data=json.dumps({}))
        my_data = ast.literal_eval(result.data)
        ...
        result2 = self.client().put('/api/v1/questions/1/answers/0',
                                    headers=dict(Authorization="Bearer " + self.a_token2),
                                    content_type="application/json",
                                    data=json.dumps({}))
        my_data2 = ast.literal_eval(result2.data)
        ...
        result3 = self.client().put('/api/v1/questions/1/answers/0',
                                    headers=dict(Authorization="Bearer " + self.a_token2),
                                    content_type="application/json",
        ...
        result4 = self.client().put('/api/v1/questions/1/answers/0',
                                    headers=dict(Authorization="Bearer " + self.a_token),
                                    content_type="application/json",
                                    data=json.dumps({"content": "2 tsp per 3 cups of flour"}))
